...
2.  Adobe Sign validates that the request and the webhook URL is valid. There is a defined mechanism for this validation where Adobe Sign makes an HTTPS GET request (with the calling application's client ID in a custom HTTPS request header) to the webhook URL and the webhook URL is expected to respond in a specific way; see [Verification of intent](#verification-of-intent-of-the-webhook-url).
...
- [Notification payload for events](webhooks/webhook_events.md#notification-payload-for-events)
- [Receiving a webhook notification](webhooks/webhook_events.md#receiving-a-webhook-notification)
- [Reliable delivery of webhook notifications](webhooks/webhook_events.md#reliable-delivery-of-webhook-notifications)
- [Payload info](webhooks/webhook_events.md#payload-info)
- [Webhook uniqueness](webhooks/webhook_events.md#webhook-uniqueness)
- [SuperSet events](webhooks/webhook_events.md#superset-events)
